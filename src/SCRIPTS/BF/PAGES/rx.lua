...
if apiVersion >= 1.16 then
    ...
    fields[#fields + 1] =
        { t = "Low", x = x + indent, y = inc.y(lineSpacing), sp = x + sp, min = 1000, max = 2000, vals = { 6, 7 } }
    fields[#fields + 1] =
        { t = "Center", x = x + indent, y = inc.y(lineSpacing), sp = x + sp, min = 1000, max = 2000, vals = { 4, 5 } }
    fields[#fields + 1] =
        { t = "High", x = x + indent, y = inc.y(lineSpacing), sp = x + sp, min = 1000, max = 2000, vals = { 2, 3 } }
end
...
if apiVersion >= 1.20 and apiVersion <= 1.42 then
    ...
    fields[#fields + 1] =
        { t = "Interval", x = x + indent, y = inc.y(lineSpacing), sp = x + sp, min = 1, max = 50, vals = { 14 } }
end

if apiVersion >= 1.31 then
    fields[#fields + 1] =
        { t = "Cam Angle", x = x, y = inc.y(lineSpacing), sp = x + sp, min = 0, max = 90, vals = { 23 } }
end
...
elseif apiVersion >= 1.40 then
    ...
    fields[#fields + 1] = {
        ...
        max = 3,
        vals = { 29 },
        table = { [0] = "Off", "PT1", "BIQUAD", "Auto" },
    }

    if apiVersion >= 1.42 then
        fields[#fields + 1] =
            { t = "Auto Smoothness", x = x, y = inc.y(lineSpacing), sp = x + sp, min = 0, max = 50, vals = { 31 } }
    end
end
