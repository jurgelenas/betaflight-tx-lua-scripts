...
if apiVersion >= 1.36 then
    fields[#fields + 1] = {
        ...
        sp = x + sp,
        min = 0,
        max = #vtx_tables.bandTable,
        vals = { 2 },
        table = vtx_tables.bandTable,
        upd = function(self)
            self.handleBandChanUpdate(self)
        end,
    }
    fields[#fields + 1] = {
        t = "Channel",
        ...
        sp = x + sp,
        min = 1,
        max = vtx_tables.frequenciesPerBand,
        vals = { 3 },
        upd = function(self)
            self.handleBandChanUpdate(self)
        end,
...
if apiVersion >= 1.37 then
    fields[#fields + 1] = {
        ...
        vals = { 6 },
        upd = function(self)
            self.handleFreqValUpdate(self)
        end,
    }
elseif apiVersion >= 1.36 then
    fields[#fields + 1] =
        { t = "Frequency", x = x, y = inc.y(lineSpacing), sp = x + sp, min = 5000, max = 5999, ro = true }
end
...
return {
        ...
        if (self.values[2] or 0) < 0 or (self.values[3] or 0) == 0 or (self.values[4] or 0) == 0 then
            ...
            if (self.values[2] or 0) > 0 then -- band != 0
                ...
                if rFreq > 0 then
                    ...
                    self.prevChanVal =
                        clipValue(math.floor((rFreq - 5100) / 100), self.fields[2].min, self.fields[2].max)
    ...
    getNextUserFreqValue = function(self, newFreq)
        ...
        if self.freqModCounter > 65 then -- rate is fast enough; do mod speedup
            if newFreq > self.prevFreqVal then
                uFreq =
                    clipValue(newFreq + math.floor(self.freqModCounter / 65), self.fields[6].min, self.fields[6].max)
            else
                uFreq =
                    clipValue(newFreq - math.floor(self.freqModCounter / 65), self.fields[6].min, self.fields[6].max)
            end
        ...
        if (#self.values or 0) >= self.minBytes and (self.fields[6].value or 0) > 0 then
            ...
            if newFreq ~= self.prevFreqVal then
                if self.values[2] == 0 then
                    ...
                    self.prevChanVal =
                        clipValue(math.floor((self.prevFreqVal - 5100) / 100), self.fields[2].min, self.fields[2].max)
                    self.fields[2].value = self.prevChanVal
                    self.values[3] = self.prevChanVal
                else
