...
if apiVersion >= 1.16 then
    ...
    if apiVersion >= 1.37 then
        ...
        fields[#fields + 1] =
            { x = x, y = inc.y(tableSpacing.row * 1.5), min = 0, max = 255, vals = { 1 }, scale = 100 }
    ...
    if apiVersion >= 1.37 then
        ...
        fields[#fields + 1] =
            { x = x, y = inc.y(tableSpacing.row * 1.5), min = 0, max = 100, vals = { 2 }, scale = 100 }
        inc.y(tableSpacing.row * 0.5)
    end
...
if apiVersion >= 1.43 then
    fields[#fields + 1] = {
        ...
        max = 4,
        vals = { 23 },
        table = { [0] = "BF", "RF", "KISS", "ACTUAL", "QUICK" },
        postEdit = function(self)
            self.updateRatesType(self, true)
        end,
    }
end
...
if apiVersion >= 1.16 then
    ...
    if apiVersion >= 1.41 then
        fields[#fields + 1] = {
            ...
            max = 2,
            vals = { 15 },
            table = { [0] = "OFF", "SCALE", "CLIP" },
        }
        fields[#fields + 1] =
            { t = "Limit %", x = x + indent, y = inc.y(lineSpacing), sp = x + sp, min = 25, max = 100, vals = { 16 } }
    end
end
